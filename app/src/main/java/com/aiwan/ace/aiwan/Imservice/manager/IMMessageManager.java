package com.aiwan.ace.aiwan.Imservice.manager;
...
import com.aiwan.ace.aiwan.Db.entity.SessionEntity;
import com.aiwan.ace.aiwan.Imservice.callback.Packetlistener;
import com.aiwan.ace.aiwan.Imservice.entity.AudioMessage;
import com.aiwan.ace.aiwan.Imservice.entity.ImageMessage;
import com.aiwan.ace.aiwan.Imservice.entity.TextMessage;
import com.aiwan.ace.aiwan.Imservice.event.MessageEvent;
import com.aiwan.ace.aiwan.Imservice.event.PriorityEvent;
import com.aiwan.ace.aiwan.Imservice.event.RefreshHistoryMsgEvent;
import com.aiwan.ace.aiwan.Imservice.service.LoadImageService;
import com.aiwan.ace.aiwan.Imservice.support.SequenceNumberMaker;
import com.aiwan.ace.aiwan.Protobuf.IMBaseDefine;
import com.aiwan.ace.aiwan.Protobuf.IMMessage;
import com.aiwan.ace.aiwan.Protobuf.helper.EntityChangeEngine;
import com.aiwan.ace.aiwan.Protobuf.helper.Java2ProtoBuf;
import com.aiwan.ace.aiwan.Protobuf.helper.ProtoBuf2JavaBean;
...
public class IMMessageManager extends IMManager{
    ...
    public void ackReceiveMsg(MessageEntity msg){
        logger.d(Tag, "chat#ackReceiveMsg -> msg:%s", msg);
    ...
    public void onEvent(MessageEvent event){
        ...
        switch (type){
            case IMAGE_UPLOAD_FAILD:{
                logger.d(Tag, "pic#onUploadImageFaild");
    ...
    public void sendMessage(MessageEntity msgEntity) {
        logger.d(Tag, "chat#sendMessage, msg:%s", msgEntity);
        // 发送情况下 msg_id 都是0
        // 服务端是从1开始计数的
        ...
        imSocketManager.sendRequest(msgData,sid,cid,new Packetlistener(getTimeoutTolerance(messageEntity)) {
            ...
            public void onSuccess(Object response) {
                try {
                    ...
                    logger.i(Tag, "chat#onAckSendedMsg");
    ...
    public void onRecvMessage(IMMessage.IMMsgData imMsgData) {
        logger.i(Tag, "chat#onRecvMessage");
        if (imMsgData == null) {
            logger.e(Tag, "chat#decodeMessageInfo failed,cause by is null");
            return;
        }
	...
	public void sendText(TextMessage textMessage) {
        logger.i(Tag, "chat#text#textMessage");
	...
	public void sendVoice(AudioMessage audioMessage) {
		logger.i(Tag, "chat#audio#sendVoice");
    ...
    public void sendSingleImage(ImageMessage msg){
        logger.d(Tag, "ImMessageManager#sendImage ");
    ...
    public void sendImages(List<ImageMessage> msgList) {
        logger.i(Tag, "chat#image#sendImages size:%d",msgList.size());
		...
		for (ImageMessage msg : msgList) {
			logger.d(Tag, "chat#pic#sendImage  msg:%s",msg);
	...
	public void resendMessage(MessageEntity msgInfo) {
        if (msgInfo == null) {
            logger.d(Tag, "chat#resendMessage msgInfo is null or already send success!");
            return;
        }
        /**check 历史原因处理*/
        ...
        logger.d(Tag, "chat#resendMessage msgInfo %s",msgInfo);
    ...
    public List<MessageEntity> loadHistoryMsg(int pullTimes,String sessionKey,PeerEntity peerEntity){
        ...
        if (sessionEntity != null) {
            ...
            logger.i(Tag, "#loadHistoryMsg# sessionEntity is null");
    ...
    public List<MessageEntity> loadHistoryMsg(MessageEntity entity,int pullTimes){
        logger.d(Tag, "IMMessageActivity#LoadHistoryMsg");
    ...
    private List<MessageEntity> doLoadHistoryMsg(int pullTimes,final int peerId,final int peerType, final String sessionKey,int lastMsgId,int lastCreateTime,int count){
        ...
        logger.d(Tag, "LoadHistoryMsg return size is %d",resSize);
    ...
    private void doRefreshLocalMsg(RefreshHistoryMsgEvent hisEvent){
        ...
        if(localFailure){
            logger.e(Tag, "LoadHistoryMsg# all msg is failure!");
            if(hisEvent.pullTimes ==1){
    ...
    public void refreshDBMsg(int peerId,int peedType,String chatKey,int lastMsgId,int refreshCnt){
        ...
        if(msgIdList.size() == (lastMsgId-beginMsgId+1)){
            logger.d(Tag, "refreshDBMsg#do need refresh Message!,cause sizeOfList is right");
            return;
        }
        // 查找缺失的msgid
    ...
    public void onReqMsgById(IMMessage.IMGetMsgByIdRsp rsp){
        ...
        if(msgList.size() <=0){
            logger.i(Tag, "onReqMsgById# have no msgList");
            return;
        }
        ...
        for(IMBaseDefine.MsgInfo msg:msgList){
            ...
            if(entity == null){
                logger.d(Tag, "#IMMessageManager# onReqHistoryMsg#analyzeMsg is null,%s",entity);
                continue;
            }
     ...
     public void onReqHistoryMsg(IMMessage.IMGetMsgListRsp rsp){
           ...
           for(IMBaseDefine.MsgInfo msgInfo:msgList){
               ...
               if(messageEntity == null){
                   logger.d(Tag, "#IMMessageManager# onReqHistoryMsg#analyzeMsg is null,%s",messageEntity);
                   continue;
               }
    ...
    private void onImageLoadSuccess(MessageEvent imageEvent){
        ...
        logger.d(Tag, "pic#onImageUploadFinish");
