...
import com.aiwan.ace.aiwan.Imservice.callback.ListenerQueue;
import com.aiwan.ace.aiwan.Imservice.callback.Packetlistener;
import com.aiwan.ace.aiwan.Imservice.event.SocketEvent;
import com.aiwan.ace.aiwan.Imservice.network.MsgServerHandler;
import com.aiwan.ace.aiwan.Imservice.network.SocketThread;
import com.aiwan.ace.aiwan.Protobuf.IMBaseDefine;
import com.aiwan.ace.aiwan.Protobuf.base.DataBuffer;
import com.aiwan.ace.aiwan.Protobuf.base.DefaultHeader;
import com.aiwan.ace.aiwan.Protobuf.base.Header;
...
public class IMSocketManager extends IMManager {
    ...
    public IMSocketManager() {
        logger.d(Tag, "login#creating IMSocketManager");
    }
    ...
    public void sendRequest(GeneratedMessageLite requset,int sid,int cid,Packetlistener packetlistener){
        ...
        try{
            //组装包头 header
            DefaultHeader header = new DefaultHeader(sid, cid);
            ...
            logger.e(Tag, "#sendRequest#channel is close!");
        }
    }
    ...
    public void packetDispatch(ChannelBuffer channelBuffer){
        ...
        logger.d(Tag, "dispatch packet, serviceId:%d, commandId:%d", serviceId,
                commandId);
       ...
       if(listener!=null){
        ...
        switch (serviceId){
            ...
            case IMBaseDefine.ServiceID.SID_GROUP_VALUE:
                ...
                break;
            default:
                logger.e(Tag, "packet#unhandled serviceId:%d, commandId:%d", serviceId,
                        commandId);
                break;
        }
    }
    ...
    public void reqMsgServerAddrs() {
        ...
        client.setUserAgent("Android-LeWan");
        client.get(SystemConfigSp.instance().getStrConfig(SystemConfigSp.SysCfgDimension.LOGINSERVER), new BaseJsonHttpResponseHandler(){
            ...
            public void onSuccess(int statusCode, cz.msebera.android.httpclient.Header[] headers, String rawJsonResponse, Object response) {
                logger.d(Tag, "socket#req msgAddress onSuccess, response:%s", rawJsonResponse);
                MsgServerAddrsEntity msgServer = (MsgServerAddrsEntity) response;
                if(msgServer == null){
            ...
            public void onFailure(int statusCode, cz.msebera.android.httpclient.Header[] headers, Throwable throwable, String rawJsonData, Object errorResponse) {
                logger.d(Tag, "socket#req msgAddress Failure, errorResponse:%s", errorResponse);
                triggerEvent(SocketEvent.REQ_MSG_SERVER_ADDRS_FAILED);
            }
    ...
    public void disconnectMsgServer() {
        ...
        if (msgServerThread != null) {
            msgServerThread.close();
            msgServerThread = null;
            logger.i(Tag, "login#do real disconnectMsgServer ok");
        }
    }
