...
public class IMUnreadMsgManager extends IMManager {
    ...
    private void reqUnreadMsgContactList() {
        logger.i(Tag, "unread#1reqUnreadMsgContactList");
    ...
    public void onRepUnreadMsgContactList(IMMessage.IMUnreadMsgCntRsp unreadMsgCntRsp) {
        logger.i(Tag, "unread#2onRepUnreadMsgContactList");
        ...
        logger.i(Tag, "unread#unreadMsgCnt:%d, unreadMsgInfoCnt:%d",unreadInfoList.size(),totalUnreadCount);
	...
	public void add(MessageEntity msg) {
        ...
        //更新未读消息计数
        if(msg == null){
            logger.d(Tag, "unread#unreadMgr#add msg is null!");
            return;
        }
        // isFirst场景:出现一条未读消息，出现小红点，需要触发 [免打扰的情况下]
        boolean isFirst = false;
		logger.d(Tag, "unread#unreadMgr#add unread msg:%s", msg);
    ...
    public void ackReadMsg(MessageEntity entity){
        logger.d(Tag, "chat#ackReadMsg -> msg:%s", entity);
    ...
    public void ackReadMsg(UnreadEntity unreadEntity){
        logger.d(Tag, "chat#ackReadMsg -> msg:%s", unreadEntity);
        int loginId = loginManager.getLoginId();
    ...
    public void onNotifyRead(IMMessage.IMMsgDataReadNotify readNotify){
        logger.d(Tag, "chat#onNotifyRead");
        //发送此信令的用户id
        int trigerId = readNotify.getUserId();
        int loginId = IMLoginManager.instance().getLoginId();
        if(trigerId != loginId){
            logger.i(Tag, "onNotifyRead# trigerId:%s,loginId:%s not Equal",trigerId,loginId);
            return ;
        }
        ...
        if(unreadSession!=null && unreadSession.getLaststMsgId() <= msgId){
            // 清空会话session
            logger.d(Tag, "chat#onNotifyRead# unreadSession onLoginOut");
    ...
    public UnreadEntity findUnread(String sessionKey){
        logger.d(Tag, "unread#findUnread# buddyId:%s", sessionKey);
        if(TextUtils.isEmpty(sessionKey) || unreadMsgMap.size()<=0){
            logger.i(Tag, "unread#findUnread# no unread info");
            return null;
        }
        if(unreadMsgMap.containsKey(sessionKey)){
