...
import com.aiwan.ace.aiwan.Imservice.event.ReconnectEvent;
import com.aiwan.ace.aiwan.Imservice.event.SocketEvent;
import com.aiwan.ace.aiwan.Utils.Logger;
import com.aiwan.ace.aiwan.Utils.NetworkUtil;
...
public class IMReconnectManager extends IMManager {
    ...
    public void onLocalLoginOk(){
       logger.d(Tag, "reconnect#LoginEvent onLocalLoginOk");
       ...
       logger.d(Tag, "reconnect#register actions");
       ctx.registerReceiver(imReceiver, intentFilter);
    }

    /** 网络链接成功*/
    public void onLocalNetOk(){
      logger.d(Tag, "reconnect#onLoginSuccess 网络链接上来,无需其他操作");
      status = ReconnectEvent.SUCCESS;
    }
    ...
    @Override
    public void reset() {
        logger.d(Tag, "reconnect#reset begin");
        try {
            EventBus.getDefault().unregister(inst);
            ctx.unregisterReceiver(imReceiver);
            status = ReconnectEvent.NONE;
            isAlarmTrigger = false;
            logger.d(Tag, "reconnect#reset stop");
        }catch (Exception e){
            logger.e(Tag, "reconnect#reset error:%s",e.getCause());
        }finally {
            releaseWakeLock();
        }
    }
    ...
    public void onEventMainThread(SocketEvent socketEvent){
        logger.d(Tag, "reconnect#SocketEvent event:%s",socketEvent.name());
    ...
    public void onEventMainThread(LoginEvent event){
        logger.d(Tag, "reconnect#LoginEvent event: %s",event.name());
        switch (event){
            ...
            switch (msg.what){
                case HANDLER_CHECK_NETWORK:{
                      if(!NetworkUtil.isNetWorkAvalible(ctx)){
                          logger.w(Tag, "reconnect#handleMessage#网络依旧不可用");
                          releaseWakeLock();
    ...
    private  void tryReconnect(){
        ...
        if(netinfo == null){
            // 延迟检测
            logger.w(Tag, "reconnect#netinfo 为空延迟检测");
        ...
        synchronized(IMReconnectManager.this) {
            ...
            if (netinfo.isAvailable()) {
                ...
                if (status == ReconnectEvent.NONE
                    ...
                    logger.i(Tag, "reconnect#无需启动重连程序");
                    return;
                }
                if (isReconnecting()) {
                    /**升级时间，部署下一次的重练*/
                    logger.d(Tag, "reconnect#正在重连中..");
                    incrementReconnectInterval();
                    scheduleReconnect(reconnectInterval);
                    logger.d(Tag, "reconnect#tryReconnect#下次重练时间间隔:%d", reconnectInterval);
                    return;
                }
                ...
                //通知上层UI修改
                logger.d(Tag, "reconnect#网络不可用!! 通知上层");
                status = ReconnectEvent.DISABLE;
                EventBus.getDefault().post(ReconnectEvent.DISABLE);
            }
        }
	...
	private void scheduleReconnect(int seconds) {
		logger.d(Tag, "reconnect#scheduleReconnect after %d seconds", seconds);
		...
		if (pi == null) {
			logger.e(Tag, "reconnect#pi is null");
			return;
		}
    ...
    private void resetReconnectTime() {
        logger.d(Tag, "reconnect#resetReconnectTime");
    ...
    public void onAction(String action, Intent intent) {
        logger.d(Tag, "reconnect#onAction action:%s", action);
        if (action.equals(ConnectivityManager.CONNECTIVITY_ACTION)) {
            logger.d(Tag, "reconnect#onAction#网络状态发生变化!!");
            tryReconnect();
    ...
    private void handleReconnectServer() {
        logger.d(Tag, "reconnect#handleReconnectServer#定时任务触发");
        acquireWakeLock();
        IMSocketManager.instance().disconnectMsgServer();
        if (isAlarmTrigger) {
            isAlarmTrigger = false;
            logger.d(Tag, "reconnect#定时器触发重连。。。");
            ...
            logger.d(Tag, "reconnect#正常重连，非定时器");
            IMSocketManager.instance().reconnectMsg();
        }
    }
    ...
    private void acquireWakeLock() {
        try {
            if (null == wakeLock) {
                ...
                logger.i(Tag, "acquireWakeLock#call acquireWakeLock");
    ...
    private void releaseWakeLock() {
        try {
            if (null != wakeLock && wakeLock.isHeld()) {
                logger.i(Tag, "releaseWakeLock##call releaseWakeLock");
                wakeLock.release();
                wakeLock = null;
            }
        }catch (Exception e){
