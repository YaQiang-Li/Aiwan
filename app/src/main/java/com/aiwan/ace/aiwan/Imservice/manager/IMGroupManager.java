...
import com.aiwan.ace.aiwan.Db.entity.GroupEntity;
...
import com.aiwan.ace.aiwan.Imservice.event.GroupEvent;
import com.aiwan.ace.aiwan.Imservice.event.SessionEvent;
import com.aiwan.ace.aiwan.Protobuf.IMBaseDefine;
import com.aiwan.ace.aiwan.Protobuf.IMGroup;
...
import com.aiwan.ace.aiwan.Utils.pinyin.PinYin;
...
public class IMGroupManager extends IMManager {
    ...
    private void loadSessionGroupInfo(){
        logger.i(Tag, "group#loadSessionGroupInfo");
    ...
    private void reqGetNormalGroupList() {
        logger.i(Tag, "group#reqGetNormalGroupList");
    ...
    public void reqGetGroupDetailInfo(List<IMBaseDefine.GroupVersionInfo> versionInfoList){
        logger.i(Tag, "group#reqGetGroupDetailInfo");
        if(versionInfoList == null || versionInfoList.size()<=0){
            logger.e(Tag, "group#reqGetGroupDetailInfo# please check your params,cause by empty/null");
            return ;
        }
        int loginId = imLoginManager.getLoginId();
    ...
    public void onRepGroupDetailInfo(IMGroup.IMGroupInfoListRsp groupInfoListRsp){
        logger.i(Tag, "group#onRepGroupDetailInfo");
        int groupSize = groupInfoListRsp.getGroupInfoListCount();
        int userId = groupInfoListRsp.getUserId();
        int loginId = imLoginManager.getLoginId();
        logger.i(Tag, "group#onRepGroupDetailInfo cnt:%d",groupSize);
        if(groupSize <=0 || userId!=loginId){
            logger.i(Tag, "group#onRepGroupDetailInfo size empty or userid[%d]≠ loginId[%d]",userId,loginId);
            return;
        }
    ...
    public void reqCreateTempGroup(String groupName, Set<Integer> memberList){
        ...
        logger.i(Tag, "group#reqCreateTempGroup, tempGroupName = %s", groupName);
        ...
        imSocketManager.sendRequest(groupCreateReq, sid, cid,new Packetlistener() {
            ...
            public void onSuccess(Object response) {
                try {
                    ...
                    logger.e(Tag, "reqCreateTempGroup parse error");
    ...
    public void onReqCreateTempGroup(IMGroup.IMGroupCreateRsp groupCreateRsp){
        logger.d(Tag, "group#onReqCreateTempGroup");

        int resultCode = groupCreateRsp.getResultCode();
        if(0 != resultCode){
            logger.e(Tag, "group#createGroup failed");
            triggerEvent(new GroupEvent(GroupEvent.Event.CREATE_GROUP_FAIL));
            return;
        }
    ...
    private void reqChangeGroupMember(int groupId,IMBaseDefine.GroupModifyType groupModifyType, Set<Integer> changeMemberlist) {
        logger.i(Tag, "group#reqChangeGroupMember, changeGroupMemberType = %s", groupModifyType.toString());
        ...
        imSocketManager.sendRequest(groupChangeMemberReq, sid, cid,new Packetlistener() {
            ...
            public void onSuccess(Object response) {
                try {
                    ...
                    logger.e(Tag, "reqChangeGroupMember parse error!");
    ...
    public void reqShieldGroup(final int groupId,final int shieldType){
        ...
        if(entity == null){
            logger.i(Tag, "GroupEntity do not exist!");
            return;
        }
	...
	public GroupEntity findGroup(int groupId) {
		logger.d(Tag, "group#findGroup groupId:%s", groupId);
        if(groupMap.containsKey(groupId)){
	...
	public List<UserEntity> getGroupMembers(int groupId) {
		logger.d(Tag, "group#getGroupMembers groupId:%s", groupId);

		GroupEntity group = findGroup(groupId);
		if (group == null) {
			logger.e(Tag, "group#no such group id:%s", groupId);
			return null;
		}
		...
		for (Integer id : userList) {
			...
			if (contact == null) {
				logger.e(Tag, "group#no such contact id:%s", id);
				continue;
			}
